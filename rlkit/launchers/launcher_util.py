...
def create_log_dir(exp_prefix, exp_id=None, seed=0, base_log_dir=None, setting=None):
    """
    ...
    if setting is None:
        ...
        exp_name = setting
    seed = "s"+str(seed)
    log_dir = osp.join(base_log_dir, exp_prefix.replace("_", "-"), exp_name, seed)
...
def setup_logger(
    ...
    if first_time:
        log_dir = create_log_dir(exp_prefix, exp_id=exp_id, seed=seed,
                                 base_log_dir=base_log_dir, setting=setting)
